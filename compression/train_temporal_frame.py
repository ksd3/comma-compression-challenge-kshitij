#!/usr/bin/env python3
"""Train temporal-frame model for commaVQ compression."""
...
from compression.temporal_frame_model import TemporalFrameModel, TFConfig, TF_CONFIGS

DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'


class TFDataset(torch.utils.data.Dataset):
    """Yields (prev_frames, curr_frame) pairs."""
...
def train(args):
    config = TF_CONFIGS[args.size]
    model = TemporalFrameModel(config).to(DEVICE)
    n_params = model.count_params()
    print(f"Model: {args.size} (dim={config.dim}, layers={config.n_layer}, K={config.context_len})", flush=True)
    ...
    dataset = TFDataset(segments, config, samples_per_epoch=args.samples_per_epoch)
    ...
    for epoch in range(args.epochs):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_path = os.path.join(os.path.dirname(__file__), f'tf_{args.size}.pt')
            torch.save({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--size', default='small', choices=list(TF_CONFIGS.keys()))
