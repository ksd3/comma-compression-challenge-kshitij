// WRITEUP.typ -- commaVQ Compression Challenge Technical Writeup

#set document(
  title: "Neural Lossless Compression of VQ-Encoded Driving Video",
  author: "Technical Writeup",
...
#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 [
      ...
      _commaVQ Compression Challenge -- Technical Writeup_
      #h(1fr)
...
#align(center)[
  #v(1cm)
  #text(20pt, weight: "bold")[Neural Lossless Compression of\ VQ-Encoded Driving Video]
  #v(0.5cm)
  #text(12pt, fill: gray)[commaVQ Compression Challenge -- Technical Writeup]
  #v(0.3cm)
...
#rect(
  ...
  #text(11pt, weight: "bold")[Summary.]
  This writeup describes an approach to the commaVQ compression challenge: losslessly compress 5,000 segments of VQ-encoded driving video into the smallest possible zip file. The final submission achieves a *2.75$times$ compression ratio* (333 MB zip from 915 MB of token data), using a 4-million parameter frame-level autoregressive transformer paired with ANS entropy coding. The interesting part is not the final number but what I learned getting there: that spatial context dominates temporal context, that tiny models beat giant ones when you can train on the test set, and that the gap between "should work in theory" and "actually works in practice" is where all the interesting problems live.
]
...
The dataset consists of 5,000 driving segments. Each segment has 1,200 frames, and each frame is an $8 times 16$ grid of discrete tokens from a vocabulary of 1,024 (10 bits per token). That is 768 million tokens total, roughly 915 MB at 10 bits each.
...
The core theoretical observation is Shannon's source coding theorem: a probabilistic model assigning probability $p(x)$ to symbol $x$ can compress it to $-log_2 p(x)$ bits. Better prediction = shorter codes. The entire project reduces to: build the best predictor that fits in a few megabytes.

= Phase 1: Classical Baselines

Before training any models, I established baselines with classical compression.

*LZMA with transposition (1.61$times$).* The single most impactful classical trick is to transpose the data. Each segment is $(1200, 8, 16)$. Reshaping to $(128, 1200)$ and transposing puts each spatial position's temporal sequence contiguously in the byte stream. LZMA's sliding-window dictionary then captures temporal repetitions efficiently.

*Delta encoding (1.28$times$).* My first instinct was to delta-encode consecutive frames, since driving video has high temporal redundancy. This actually _hurt_. The reason is structural: VQ token indices are categorical, not ordinal. Token 500 and token 501 represent arbitrarily different visual patterns. Frame-to-frame differences of categorical labels produce high-entropy residuals, unlike pixel-space deltas where differences are typically small.

*10-bit packing (1.29$times$).* Also counterproductive. Packing 10-bit values into a dense bitstream breaks the byte alignment that LZMA depends on for dictionary matching. The "wasted" 6 bits per token in int16 storage actually help LZMA by maintaining alignment.
...
Adjacent frames at the same spatial position share 5.40 bits of mutual information (out of 10 bits total entropy). This decays roughly logarithmically with temporal distance: 1.53 bits at distance 20, 1.24 bits at distance 40.

== Conditional Entropy Ceilings

I measured $H("token" | "previous" K "tokens at same position")$ for $K = 1, 2, 3, 5, 10, 20$. Even $K = 20$ only reduces the conditional entropy to about 4.6 bits/token when using a single position's history.

== Spatial Context: The Key Observation

This was the most important measurement of the project. I computed conditional entropy under different spatial conditioning:
...
The joint conditioning reduces entropy from 10 bits to 1.45 bits, an 85% reduction. What surprised me was that spatial context from the _current_ frame (3.39 bits) is actually _more informative_ than temporal context from the _previous_ frame (4.62 bits). This fundamentally reoriented the project: any approach that models positions independently is leaving most of the compressible information untouched, regardless of how much temporal context it uses.
...
*Transition tables + ANS (2.16$times$).* Per-position first-order Markov tables $P("token" | "prev_token", "position")$ give 2.16$times$ compression. But the tables themselves are $tilde 50$ MB after LZMA, eating most of the data savings. The net compression was worse than the v3 neural approach which captures the same patterns in 2.3 MB of model weights.

*Factored table compression.* Using $P("token" | "prev") times P("token" | "above") / P("token")$ as a product-of-experts approximation. Gets 3.62 bits/token but tables are 49 MB. Not competitive.
...
+ *Previous frame encoder.* Each of $K = 3$ previous frames is embedded via shared token + position embeddings, producing $K times 128$ context vectors.

+ *Current frame decoder.* Transformer blocks with:
  - Causal self-attention: position $i$ attends to positions $0, ..., i-1$ (raster order)
  - Cross-attention: all positions attend to the $K times 128$ previous-frame context
  - Feed-forward network

+ *Output head.* Linear projection to 1024-way classification (vocabulary size).
...
An elegant property of autoregressive models: during compression, all tokens are known, so the causal mask lets us compute all 128 probabilities in a single forward pass (teacher forcing). Compression takes 1 forward pass per frame, or $tilde 2.7$ seconds per segment.

Decompression requires sequential decoding: predict position 0, decode from ANS, feed back, predict position 1, etc. That is 128 forward passes per frame, roughly 128$times$ slower. This asymmetry is acceptable since decompression speed is not scored, but for practical deployment I prototyped a row-level model (8 passes per frame instead of 128) that showed comparable bits/token.
...
The medium3 model is 4.5 MB larger than small3 but saves $tilde 0.09$ bits/token, which translates to roughly 8.6 MB of compressed data savings. Net gain: $tilde 4$ MB.

== Model Compression

The trained weights are stored as:
+ FP16 quantization (no quality loss observed)
+ LZMA compression of the fp16 buffer
+ Compact JSON header for architecture config
...
For each segment:
+ Run the model teacher-forced over all 1,200 frames to get probability distributions for all $1200 times 128 = 153,600$ tokens.
+ Quantize probabilities to 24-bit fixed-point integers.
+ Encode tokens in reverse order using `AnsCoder.encode_reverse()`.
+ Store the compressed bitstream.

Decompression reverses this: decode tokens forward, regenerating probabilities as each token is revealed.

Critical implementation detail: the probability distributions used during encoding and decoding must be _exactly_ identical at the bit level. Any floating-point non-determinism will cause silent decoding failures. The 24-bit quantization ensures reproducibility.
...
#align(center)[
  #table(
    ...
    [LZMA + transpose], [6.21], [1.61$times$], [Classical baseline],
    [Delta + LZMA], [7.81], [1.28$times$], [Hurt: tokens not ordinal],
    [Transition tables], [4.63], [2.16$times$], [Tables too large (49 MB)],
    [Per-position v3], [3.90], [2.40$times$], [Limited spatial context],
    [Frame small ($K = 1$)], [3.70], [$tilde$2.58$times$], [Full spatial, 1 prev frame],
    [Frame small3 ($K = 3$)], [3.61], [$tilde$2.66$times$], [3 prev frames],
    [*Frame medium3 ($K = 3$)*], [*3.56*], [*2.75$times$*], [*Final submission*],
  )
]

= What I Would Try With More Time

+ *Larger model.* A 10M parameter model ($tilde 18$ MB compact) might be net positive if it saves more than 18 MB in compressed data. The medium vs. small improvement curve suggests this is likely.

+ *Online adaptation.* Fine-tune the model on each segment before compressing it, transmitting weight deltas. This is the direction the top leaderboard entries take.

+ *INT8 quantization.* More aggressive weight quantization could shrink the model by 2$times$ with careful calibration.

+ *Non-raster orderings.* Center-out or learned orderings could provide better spatial context at each position.

+ *Row-level model.* The row model prototype (8 forward passes per frame vs. 128) showed comparable bits/token with much faster decompression. More training could make this the better choice for practical deployment.
...
python compression/frame_compress.py \
    --model compression/frame_medium3.pt --skip-verify

# The submission zip is at compression/frame_submission.zip

# Verify (WARNING: decompression is ~128x slower)
...
```bash
python compression/frame_compress.py \
    --model compression/frame_medium3.pt --num-segments 100
```
...
#table(
  ...
  [`report/report.typ`], [Typst source for detailed technical report],
